...
| 1-4 | P1 | `GetHome` 话题通过 `Pagination{PageSize:5}` 显式限 5 条（latest/hot 各 5） | ✅ |
...
| 1-2 | **P1** | 话题列表分页失效，同 1-1。 | `service/novel_post.go:58-66` |
| 1-3 | **P1** | 书评圈列表分页失效，同 1-1。 | `service/novel_book_review.go:35-44` |
| 1-4 | **P1** | 首页聚合 `GetHome` 拉取**全表话题**注入最新/热门（依赖 1-2 的错误），每次首页请求全表扫描 + 大响应；修复 1-2 后仍应显式 `Limit(5~10)`。 | `service/novel_book.go:327-338` |
...
| 2-2 | P2 | `AddComment` **未校验目标话题存在/状态**：被删帖或下架帖 id 仍可写入评论 → 孤儿评论 + `comment_count` 空增（静默）。应在事务内先锁定/校验 post 存在且 `status=正常`。 | `service/novel_post.go:377-414` |
...
| 3-4 | P3 | 楼中楼回复（`AddComment`）未校验「父评论属于同一话题」，结构与数据一致性弱。 | `service/novel_post.go:385-391` |
...
2. **物理删除 vs 软删除**：话题/书评/互动采用物理删除（需求 §2.4-6 允许软删除选项）。若需审计还原，可迁移至 `status=2` 软删除。
...
| §2.4-5 话题字数/summary/read_time | `5000~10000` 校验、前150字、`ceil(字符/400)` | ✅ |
| §2.4-6 级联删除（书→书评/书架；话题→互动/评论） | 事务级联实现 | ✅ |
...
2. ✅ 已补 2-2 话题存在性校验（事务内 `FOR UPDATE` 锁定）。
